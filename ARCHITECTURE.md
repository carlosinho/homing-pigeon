...
`PrivacyProvider` stores the display preference under `mailroom.privacyView` in `localStorage`, reading it on initialization to preserve masking after reloads. The footer toggle controls the shared `PrivateAddress` component in Messages and Senders sender cells and Domains domain cells. Enabled masking renders contiguous middle chunks of names and domain labels in the locally hosted Redacted font with opaque bars. Short prefixes and endings, address separators, and domain suffixes remain readable; labels shorter than four characters are fully covered. `PrivateSubject` applies the same shared `RedactedChunk` rendering to each subject word, preserving whitespace and the empty-subject placeholder. It omits the full-text title while privacy view is enabled. CSS keeps redacted characters covered while the font loads. Original text remains available when copied; unknown-value labels remain readable. Original data continues to drive filtering, sorting, navigation, and actions.
