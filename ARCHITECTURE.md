...
5. Add missing `activity_events.target` and `messages.category` / `messages.probable_spam` columns for older databases; create the unclassified-message index after adding the category column.
...
There is no schema-version table or general migration runner. `CREATE TABLE IF NOT EXISTS` creates a fresh database but will not evolve older tables; the activity target and message classification columns use explicit compatibility checks.
...
| `category` | Nullable Jev category; existing categories are skipped by classification. |
| `probable_spam` | Nullable integer: `1` when Jev spam probability is at least 0.7, `0` below that threshold, `NULL` when not assessed. |
...
`server/src/classification-worker.ts` runs one classification job at a time across accounts, independently of the existing Gmail worker. Within each job, four request slots share an in-memory ID cursor that claims each eligible message before awaiting Jev; each free slot immediately selects the next message. It selects unclassified messages up to the job's maximum ID, sends sender and subject through `server/src/jev.ts`, and transactionally updates the existing message's nullable `category`, `probable_spam`, and job progress. A partial index on `(account_id, id) WHERE category IS NULL` supports this selection. Updates never recreate deleted rows. Deletions can make the completed count smaller than the initial total.

Editable instructions and structured category definitions (`covers`, `not_for`, and sender/subject examples) live in `server/src/classification-guidance.ts`. The same file contains independent spam instructions and positive/negative examples, including random-looking sender addresses as supporting evidence rather than an automatic verdict. The adapter includes both questions in every request. The Jev adapter uses the HTTP API with one Choice question and seven options: `newsletter`, `marketing`, `dev_update`, `travel`, `social_media`, `purchases`, and `other`. It validates the probability values and stores the option with the highest probability without a confidence threshold. Equal maxima use the first category in the declared order. An independent Noul question returns the probability of spam; values at or above 0.7 set `probable_spam` to 1, lower values to 0. Both answers must validate before either result is saved. Only sender and subject leave the app; there is no importance assessment. The key stays on the backend.

Requests time out after 30 seconds. HTTP 429/5xx responses get up to three attempts with bounded backoff and Retry-After handling; long cooldowns, authentication failures, transport failures, and invalid responses fail the job visibly. On terminal failure, scheduling stops and outstanding requests settle, saving successful results before the job is marked failed or another job starts. Retry waits occupy their request slots. Previously saved results survive. A new manual run processes remaining unclassified messages. Startup changes interrupted `running` classification jobs to `queued`, preserving progress and the snapshot boundary. Classification results are not refreshed automatically.

`GET /api/accounts/:accountId/classification` returns whether Jev is configured, the account's unclassified count, and latest job. The frontend shows classification controls, notices, and category badges only after the status endpoint confirms that Jev is configured. Without a key, it uses the ordinary action-column width and hides even previously saved categories without deleting them. The CLASSIFY toggle beside More columns reveals page/all classification actions and a note explaining scope and data sent. The Messages screen polls this endpoint every 500 ms during active classification (5 seconds when idle or after a polling error) and refreshes visible rows as progress changes, including when the first poll already reports completion. Progress refreshes bypass the 220 ms debounce once the current message query has settled; filter changes retain their debounce. Badges sit in the last column before the Gmail link; flagged messages show only ☠️ beside the category with a “probably spam” title and accessible label. The emoji follows the same API-key visibility rule. Message JSON exposes `category` and nullable boolean `probable_spam`; CSV uses 1/0/blank for the spam flag. Category and spam sorting/filtering are not added.

`DELETE /api/classifications` resets message categories and spam assessments to NULL across every account with one SQLite update. It returns 409 if any classification job is queued or running, preventing pending Jev results from repopulating erased classifications. It requires no Jev API key and preserves messages and job history. The UI places this action at the far right of the classification toolbar with inline two-click confirmation, then refreshes categories and status after success.
...
`PrivacyProvider` stores the display preference under `mailroom.privacyView` in `localStorage`, reading it on initialization to preserve masking after reloads. The footer toggle controls the shared `PrivateAddress` component in the Fetch summary, Messages and Senders sender cells, and Domains domain cells. The top mailbox selector uses an equivalent plain-text mask because native option elements cannot contain the styled redaction spans. Enabled masking renders contiguous middle chunks of names and domain labels as opaque bars. Short prefixes and endings, address separators, and domain suffixes remain readable; labels shorter than four characters are fully covered. `PrivateSubject` applies the same shared `RedactedChunk` rendering to each subject word, preserving whitespace and the empty-subject placeholder. It omits the full-text title while privacy view is enabled. CSS keeps styled redacted characters covered while the font loads. Original text remains available when styled redactions are copied; unknown-value labels remain readable. Original data continues to drive account selection, filtering, sorting, navigation, and actions.
