...
The footer’s **Privacy view** toggle partially redacts connected mailbox addresses in the top selector and Fetch summary, sender addresses in the Messages and Senders tables, domain names in the Domains table, and subjects in Messages. Contiguous middle chunks of names and domain labels appear as solid bars using the locally hosted Redacted font; the native mailbox selector uses equivalent plain-text blocks. Short prefixes and endings, address separators, and domain suffixes stay readable; short labels are fully covered. Subjects use the same chunk redaction within each word, preserving spaces; their full-text hover tooltip is disabled while privacy view is on. Copying styled redactions preserves the original text. It defaults to off and remembers your choice across page navigation and reloads. This is display-only: filters, Fetch activity, links, and CSV exports retain their original values.
